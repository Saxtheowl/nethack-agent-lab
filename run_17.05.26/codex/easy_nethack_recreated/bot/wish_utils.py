...
def ensure_claimed_wish(claim_dir, session_name, char_name):
...
def list_wish_ready_sessions(data_dir):
...
def cmd_ensure_claim(args):
    claim_dir, session_name, char_name = args
    record = ensure_claimed_wish(claim_dir, session_name, char_name)
    print(record["label"])
    return 0
...
def cmd_list_ready(args):
    (data_dir,) = args
    for record in list_wish_ready_sessions(data_dir):
        print(
            "\t".join(
                [record["session_name"], record["char_name"], record["path"]]
            )
        )
    return 0
...
def main(argv):
    commands = {
        ...
        "ensure-claim": (3, cmd_ensure_claim),
        "list-ready": (1, cmd_list_ready),
