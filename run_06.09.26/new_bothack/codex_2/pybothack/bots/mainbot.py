...
def full_explore(game):
    ...
    if get_level(game, 'main', 'sanctum'):
        return None
    res = explore(game, 'mines', 'minetown')
    if res is None:
        res = explore(game, 'main', 'sokoban')
    if res is None and have(game, "Excalibur") and have_throwable(game):
        res = do_soko(game)
    if res is None:
...
def handle_impairment(game):
    ...
    if res is None:
        ...
        # Keep the blindfold on while Medusa is still tracked on this level.
        # The combat handler equips it before attacking, but this impairment
        # handler runs first on the next decision and used to remove it again,
        # allowing the following turn to meet Medusa's gaze.
        medusa_present = any(typename(m) == "Medusa"
                             for m in curlvl_monsters(game))
        if found and not medusa_present:
...
def unlockable_chest(game, tile):
    return bool(not shop(tile) and not in_gehennom(game)
                and branch_key(game) != 'vlad'
                and any(i.get('locked') for i in (tile.get('items') or ()))
...
def drop_junk(game):
    ...
    if found:
        slot, item = found
        return with_reason("dropping junk",
...
def avoid_medusa(game, monster):
    """Never take a normal melee turn against Medusa.

    A Medusa can petrify the hero on the same turn as a successful hit, so
    the generic adjacent-monster combat path must not be allowed to fall
    through to Attack().
    """
    if typename(monster) != "Medusa":
        return None
    found = have(game, set(blind_tool), {'noncursed'})
    if found:
        return with_reason("blind before Medusa", make_use(game, found[0]))
    player = game['player']
    p = navigate(game,
                  lambda t: distance(t, monster) > distance(player, monster),
                  {'no-fight': True, 'no-traps': True, 'max-steps': 4})
    return with_reason("avoid Medusa", p['step'] if p else None)


def hit(game, level, monster):
    player = game['player']
    res = avoid_medusa(game, monster)
    if res is None:
...
def engrave_e(game, perma=False, farm=False):
    player = game['player']
    tile = at_player(game)
    append = e_p(tile) or less_than(200, tile.get('engraving') or "")
...
def pray_for_hp(game):
    player = game['player']
    if _can_pray(game) and (player['hp'] < 6
                            or player['maxhp'] // 7 >= player['hp']):
        from ..actions import Pray
...
def _hunt_action(game, robbed_of, allow_blindfold=True):
    player = game['player']
    if not robbed_of:
        return None
    ...
    if res is None and allow_blindfold \
            and (not recent or game['turn'] - recent['known'] > 11) \
...
def hunt(bh):
    robbed_of = []
    # Telepathy scouting can otherwise oscillate forever: hunt equips a
    # blindfold, the impairment handler immediately removes it, and hunt
    # equips it again on the next decision.  One scouting attempt per window
    # is enough; wandering and the normal timeout can then resolve the theft.
    last_blindfold = [-100000]

    def choose_action(game):
        before = len(robbed_of)
        allow_blindfold = not before or game['turn'] - last_blindfold[0] >= 500
        action = _hunt_action(game, robbed_of, allow_blindfold)
        if allow_blindfold and action and 'make use' in str(action).lower():
            last_blindfold[0] = game['turn']
        return action
...
def use_items(game):
    ...
    if less_than(50, inventory(game)):
        ...
        # An empty wand reports "Nothing happens" but remains in inventory.
        # Retrying it forever prevents progress at the castle or Sanctum when
        # no charging scroll is available.
        can_recharge_wow = (have(game, "scroll of charging",
                                  {'blessed', 'bagged'})
                            or have(game, "scroll of charging",
                                    {'wished', 'bagged'}))
        recent_wow = typekw(game.get('last-action')) == 'zapwand'
        if found and (not recent_wow or can_recharge_wow):
            slot, wow = found
...
def farm_done(game):
    ...
    if visited(game, 'wiztower'):
        return True
    if (game.get('score') or 0) > 15000000:
        return True
    # A live fallback for runs that have already paid for the farm but missed
    # one of the consumable bookkeeping conditions below.  Without this, a
    # successful pudding farm can remain on its sink indefinitely: the player
    # is powerful enough to survive, but the missing inventory predicate keeps
    # `farm()` ahead of `progress` forever.  At this point the accumulated
    # score and turn count are already far beyond the intended preparation
    # phase, so continuing to farm has a worse expected outcome than resuming
    # the route to the Amulet.
    if (game.get('score') or 0) > 5000000 and game.get('turn', 0) > 40000:
        return True
    # A late farming setup can deadlock while digging around a sink.  Once a
    # run has already reached the late dungeon with a healthy score, resume
    # the route and give it a chance to reach the endgame.
    if ((game.get('score') or 0) > 100000
            and game.get('turn', 0) > 15000
            and game.get('dlvl') not in (None, 'Dlvl:1')):
        return True
    return bool(
        (game.get('score') or 0) > 6000000
        and (game['wishes'] >= 3
             or ((have_levi(game) or have(game, "speed boots")
                  or game['turn'] > 65000)
                 and (have_mr(game) or "L" in game['genocided']
                      or game['turn'] > 65000)
                 and reflection(game) and game['player']['ac'] < -10))
        and have(game, bag)
        and have_sum(game, "scroll of remove curse", {'bagged'}) >= 6
        and have(game, "scroll of identify", {'bagged'})
        and (have_candles(game) or not get_level(game, 'main', 'castle')))
