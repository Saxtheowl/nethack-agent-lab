...
_MENU_FNS = [
    (r"What do you wish to do\?", 'name_menu'),
    ...
    # NetHack uses this heading after a look/recovery action on a tile with
    # several objects.  It is the same inventory-shaped menu, and leaving it
    # unclassified strands the scraper in `marked` until the watchdog quits.
    (r"Things that are here:", 'inventory_list'),
    (r"There are several objects here\.", 'inventory_list'),
]
...
def _multi_menu(head):
    return not re_seq(r"What do you wish to do\?|Pick a skill", head)
...
BOTL2_RE = (r"^(Dlvl:\d+|Home \d+|Fort Ludios|End Game|Astral Plane)\s+"
...
def new_scraper(delegator, no_mark_prompt=None):
    ...
    def marked(frame):
        # A travel prompt can arrive after a marked redraw.  It used to be
        # handled only by the no-mark scraper, leaving the real game waiting
        # forever after `Where do you want to travel to?` while the reader
        # continued to receive redraws.  Treat location prompts at the same
        # stage as the other interactive prompts so the temporary travel
        # handler can answer and restore the normal scraper.
        for f in (handle_game_end, handle_more, handle_menu,
                  handle_choice_prompt, handle_prompt, handle_location):
            r = f(frame)
            if r:
                return r
