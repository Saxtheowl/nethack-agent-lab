...
def main():
    ...
    # Guard against the two-seeds trap.  `--seed` is the BOT's RNG seed; the
    # corpus directories are named after the NETHACK seed.  Passing the latter
    # gives the port an RNG stream the original never had, and the result is a
    # single flipped coin-toss deep into an otherwise byte-identical stream -
    # which reads exactly like a fidelity regression and cost a full
    # investigation before the recording was re-checked.  The sibling
    # recording.json knows both, so ask it.
    _sib = os.path.join(os.path.dirname(os.path.abspath(a.tap_log)),
                        'recording.json')
    if os.path.exists(_sib):
        try:
            _rec = json.load(open(_sib))
        except (ValueError, IOError):
            _rec = {}
        _bot = _rec.get('bot_seed')
        if _bot is None and _rec.get('seed') == a.seed and a.seed != 12345:
            ap.error("--seed %d is this recording's NETHACK seed; the bot's "
                     "RNG seed is what this flag wants (default 12345, from "
                     "BOTHACK_SEED at record time).  Re-run without --seed."
                     % a.seed)
        if _bot is not None and _bot != a.seed:
            ap.error("--seed %d disagrees with the recording's bot_seed %d "
                     "(%s).  The replay would use an RNG stream the original "
                     "never had." % (a.seed, _bot, _sib))
