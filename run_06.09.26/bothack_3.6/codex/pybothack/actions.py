...
def Pray():
    def handler(a, bh):
        bh.game.swap(adjust_prayer_timeout)
        return Handler(confirm_pray=lambda _prompt: True)
...
def Name(slot, name):
    def hnd(a, bh):
        update_inventory(bh)
        return Handler(name_menu=lambda options: next(k for k, v in options.items() if 'particular object' in v),
...
def Call(slot, name):
    def hnd(a, bh):
        ...
        return Handler(name_menu=lambda options: next(k for k, v in options.items() if 'type of an object in inventory' in v),
                       name_what=lambda _t: slot,
                       what_name=lambda _t: name)
        ...
        def pick_up_what(options):
            ...
            for slot, lbl in options.items():
                if lbl in remaining:
                    ...
                    res.add(slot)
            return res
        return Handler(pick_up_what=pick_up_what)
