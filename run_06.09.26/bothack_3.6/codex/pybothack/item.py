...
ITEM_RE = re.compile(
    ...
    r"(\((?:in quiver(?: pouch)?|at the ready)\))?\s*(\(altern.*?\)?)?\s*(\(wielded i.*?\))?\s*"
...
def parse_label(label):
    ...
    # NetHack 3.6 adds container knowledge before the BUC adjective.
    container_state = re.search(r'\b(empty|unlocked|locked) (?=(?:blessed |uncursed |cursed )?(?:bag|sack|chest|large box|ice box|oilskin sack))', norm)
    if container_state:
        norm = norm[:container_state.start()] + norm[container_state.end():]
    ...
    if container_state:
        if container_state[1] == 'empty':
            res['items'] = []
        else:
            res['locked'] = container_state[1] == 'locked'
    out = {'label': label}
