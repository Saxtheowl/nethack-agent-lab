...
def consider_items(game):
    if game.get('recovery-items-until', 0) > game.get('turn', 0):
        return None
...
def engrave_e(game, perma=False, farm=False):
    # Disabled: 3.4 combat and stacking tactics violate the 3.6 conditions.
    return None


def pray_for_hp(game):
    player = game['player']
    from ..rules367 import critically_low_hp
    if _can_pray(game) and critically_low_hp(player['hp'], player['maxhp'], player['xplvl']):
        from ..actions import Pray
...
def farming(game):
    # Clones no longer sustain the 3.4 loot/XP economy.
    return False


def farm_done(game):
    # Readiness is equipment based; score and pudding yield are not resources.
    return bool(have_mr(game) and reflection(game)
                and have_levi(game) and have_pick(game)
                and have(game, bag) and game['player']['ac'] < -10)
