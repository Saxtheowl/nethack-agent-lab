...
def parse_label(label):
    # 3.6.7 names are normalised for parsing only: 'label' keeps the text the
    # game prints, because handlers compare it with menu entries verbatim
    # (e.g. PickUp matches "Pick up what?" options against item labels)
    from .compat36 import normalize_label
    raw_label = label
    ...
    if re.search(r'\d+ zorkmids? each', label):
        res['cost-each'] = True       # unit price (see game.itemid_handler)
    out = {'label': raw_label}
