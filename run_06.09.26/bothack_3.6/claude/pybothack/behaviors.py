...
def attach_candles(game):
    ...
    if not c:
        # port: the original asserted; reached when every level was explored
        # without getting the Candelabrum (big-w01 g007, g019, g048 crashed)
        return None
...
def invocation(game):
    if get_level(game, 'main', 'sanctum'):
        return None
    missing = [x for x in (BELL, CANDELABRUM, BOOK)
               if not have(game, x, {'bagged'})]
    if missing:
        key = tuple(missing)
        if key not in _MISSING_LOGGED:
            _MISSING_LOGGED.add(key)
            import logging
            logging.getLogger('bothack.behaviors').warning(
                "invocation impossible, missing %s", missing)
        return None
