...
def e_p(tile):
    """Is a valid Elbereth inscribed on the tile?  3.6.7 engrave.c
    sengr_at(..., strict=TRUE): the whole text must be Elbereth."""
    from .rules36 import strict_elbereth_text
    return strict_elbereth_text(tile.get('engraving'))
...
def engravable(tile):
    return (walkable(tile)
            and not (pool_p(tile) or lava_p(tile) or fountain_p(tile)
                     ...
                     # "You can't reach the bottom of the pit." (big-w01 g027)
                     or tile.get('feature') in ('pit', 'spikepit', 'hole',
                                                'trapdoor', 'web'))
